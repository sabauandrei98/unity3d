...
public class CreateObstacleEasy : MonoBehaviour {
	...
	float minDistanceBetweenPipesHeight = 2f;

	//Wall prefab
	public GameObject wall;

	float lastPipeHeight;
	...
	void Start () {
		...
		lastPipeHeight = lineActionInterval;
	}
	...
	float GetWallHeight()
	{
		...
		while (absoluteValue (lastPipeHeight - randomPipeY) < minDistanceBetweenPipesHeight) {
	...
	{
		if (distanceBetweenPipes <= 1f)
			return;
		distanceBetweenPipes -= 0.04f;
	}
